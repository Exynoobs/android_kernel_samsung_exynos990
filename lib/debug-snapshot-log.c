...
int secdbg_show_busy_task(unsigned int cpu, unsigned long long duration, int count)
{
	...
	pr_auto(ASL5, "CPU%u [CFS util_avg:%u load_avg:%u nr_running:%u][RT util_avg:%u load_avg:%u rt_nr_running:%u]",	\
		cpu, cpu_rq(cpu)->cfs.avg.util_avg, cpu_rq(cpu)->cfs.avg.load_avg, cpu_rq(cpu)->cfs.nr_running,	\
		cpu_rq(cpu)->rt.avg.util_avg, cpu_rq(cpu)->rt.avg.load_avg, cpu_rq(cpu)->rt.rt_nr_running);
